...
// 
// Параметры:
//  ДанныеТелеграм - Соответствие из КлючИЗначение - Данные телеграм
//	ДополнительныеПараметры - Структура - Дополнительно передаваемые данные
//	 
...
Функция ПолучитьОбъектПоДаннымТелеграм(ДанныеТелеграм, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ЗначенияРеквизитов = ПолучитьРеквизиты(Объект.Ссылка);	
	...
	Для каждого РеквизитОбъекта Из РеквизитыОбъекта Цикл
		...
		Если ЗначенияРеквизитов.Свойство(РеквизитОбъекта.Ключ)
			И ЗначенияРеквизитов[РеквизитОбъекта.Ключ] = РеквизитОбъекта.Значение Тогда
			Продолжить;
		КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("Вебхук") 
		...
		Если ИдФайла <> Неопределено Тогда
			...
			ДанныеФайла = тг_Сервер.ПолучитьФайлТелеграм(ДополнительныеПараметры.Бот, ИдФайла);
...
//  
// Возвращаемое значение:
//  СправочникСсылка.тг_Объект, Массив из СправочникСсылка.тг_Объект - Получить объект по отбору
...
Функция ЗаполнитьРеквизиты(ДанныеТелеграм) Экспорт	
	...
	Если ДанныеТелеграм["title"] <> Неопределено Тогда
		Наименование = Наименование + ДанныеТелеграм["title"];
	КонецЕсли;
	
	Если РеквизитыОбъекта.Свойство("Фамилия") Тогда
		Наименование = Наименование + РеквизитыОбъекта.Фамилия;
	КонецЕсли;

	Если РеквизитыОбъекта.Свойство("Имя") Тогда
		Наименование = Наименование + ?(ПустаяСтрока(Наименование), "", " ") + РеквизитыОбъекта.Имя;
	КонецЕсли;

	Если РеквизитыОбъекта.Свойство("ИмяПользователя") Тогда
		Если ПустаяСтрока(Наименование) Тогда
			ШаблонТекста = "%1";
		Иначе
			ШаблонТекста = " (%1)";
		КонецЕсли;
		Наименование = Наименование + СтрШаблон(ШаблонТекста, РеквизитыОбъекта.Имя);
	КонецЕсли;
	
	Если ДанныеТелеграм["text"] <> Неопределено 
		И РеквизитыОбъекта.Свойство("Код") Тогда
		Наименование = "Сообщение №" + РеквизитыОбъекта.Код;
	КонецЕсли;
	
	Если ДанныеТелеграм["photo"] <> Неопределено
		И РеквизитыОбъекта.Свойство("Код") Тогда
		Наименование = "Изображение №" + РеквизитыОбъекта.Код;
	КонецЕсли;
	
	Если ДанныеТелеграм["document"] <> Неопределено
		И РеквизитыОбъекта.Свойство("Код") Тогда
		Наименование = "Изображение №" + РеквизитыОбъекта.Код;
	КонецЕсли;

	РеквизитыОбъекта.Вставить("Наименование", Наименование);
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	сФормы.Вставить(Перечисления.тг_ТипыОбъектов.Бот			, "Справочник.тг_Объект.Форма.ФормаБота");
	сФормы.Вставить(Перечисления.тг_ТипыОбъектов.Группа	 	 	, "Справочник.тг_Объект.Форма.ФормаЧата");
	сФормы.Вставить(Перечисления.тг_ТипыОбъектов.Канал			, "Справочник.тг_Объект.Форма.ФормаЧата");
	сФормы.Вставить(Перечисления.тг_ТипыОбъектов.ГруппаКанала	, "Справочник.тг_Объект.Форма.ФормаЧата");
	сФормы.Вставить(Перечисления.тг_ТипыОбъектов.Сообщение		, "Справочник.тг_Объект.Форма.ФормаСообщения");
	сФормы.Вставить(Перечисления.тг_ТипыОбъектов.Пользователь	, "Справочник.тг_Объект.Форма.ФормаПользователя");
	
	Если Параметры.Свойство("Ключ") Тогда
		ТипОбъекта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Ключ, "Тип");
		ВыбраннаяФорма = сФормы.Получить(ТипОбъекта);
	КонецЕсли;

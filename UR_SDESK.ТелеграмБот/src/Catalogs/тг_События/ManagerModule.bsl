...
Процедура ИнициализироватьКомпоновщикНастроек(ТипДанных, КомпоновщикНастроек, ИдентификаторФормы = Неопределено, Настройки = Неопределено) Экспорт
...
Функция ПолучитьСобытияОбъекта1С(Объект1С) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		стр.Вставить("Текст"			, ПолучитьТекст(Выборка.ФорматированныйТекст.Получить(),ТипОбъекта1С, Объект1С, ПараметрыСообщения));
...
Функция ШаблонСообщенияТест(Объект1С)

	Возврат "*" + "Задача" +  " " + Объект1С.Номер + " " + ПривестиТекстКMarkdownV2(Объект1С.Наименование) + "*
			|Клиент: __" + ПривестиТекстКMarkdownV2(Объект1С.Контрагент) + "__
			|
			|``` " + ПривестиТекстКMarkdownV2(Объект1С.Описание) + " ```";

КонецФункции

Функция ПривестиТекстКMarkdownV2(Текст)

	мТекст = "";
	Для сч = 1 По СтрДлина(Текст) Цикл
		ЗначениеСимвола = Сред(Текст,сч,1);
		Если КодСимвола(ЗначениеСимвола)>=1
			И КодСимвола(ЗначениеСимвола)<=126 Тогда
			мТекст = мТекст + "\";	
		КонецЕсли;
		мТекст = мТекст + ЗначениеСимвола;
	КонецЦикла;
